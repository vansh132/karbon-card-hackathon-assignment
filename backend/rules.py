...
def latest_financial_index(data: dict):
    ...
    for index, financial in enumerate(data.get("financials")):
        if financial.get("nature") == "STANDALONE":
            # print( "first financial entry index value",index)
            return index
...
def total_revenue(data: dict, financial_index):
    ...
    """
    if financial_index < len(data.get("financials")):
        financial = data.get("financials")[financial_index]
        pnl = financial.get("pnl", {})
        net_revenue = pnl.get("lineItems").get("net_revenue", 0)
        # print("net_revenue: ", pnl.get("lineItems").get("net_revenue", 0))
        return net_revenue
    return 0
...
def total_borrowing(data: dict, financial_index):
    ...
    if financial_index < len(data.get("financials")):
        financial = data.get("financials")[financial_index]
        bs = financial.get("bs", {}).get("liabilities", {})
        total_borrowings = bs.get("other_long_term_liabilities", 0) + bs.get("short_term_borrowings", 0)
        # print( "Total borrowings", total_borrowings)
        total_revenue_value = total_revenue(data, financial_index)
        if total_revenue_value:
            # print("total borrowings ratio: ", total_borrowings/total_revenue_value)
...
def iscr(data: dict, financial_index):
    ...
    if financial_index < len(data.get("financials")):
        ...
        profit_before_interest_and_tax = lineItems.get("profit_before_interest_and_tax", 0)

        depreciation = lineItems.get("depreciation", 0)

        totalSum1 = profit_before_interest_and_tax + depreciation + 1
        # print("Total Sum1: ", totalSum1)
        interest_expenses = lineItems.get("interest", 0)
        totalSum2 = interest_expenses + 1
        # print("ISCR: ", totalSum1 / totalSum2)
